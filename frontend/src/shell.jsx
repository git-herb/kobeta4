/* ============================================================
   App shell — nav, cursor, shared atoms
   ============================================================ */
import { useEffect } from "react";
import { sevLabel } from "./data.js";
...
export function useReveal() {
  useEffect(() => {
    ...
    // stalls under compositor throttling (background/offscreen tabs).
...
/* ---------- severity tag ---------- */
export function SevTag({ sev, withCode }) {
  const s = sevLabel(sev);
  return <span className={"sev " + s.cls}>{withCode ? "[" + sev + "] " : ""}{s.label}</span>;
}

/* ---------- page header ---------- */
export function PageHead({ label, title, meta }) {
  return (
