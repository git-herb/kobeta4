...
copy .env.example .env        # OPENAI_API_KEY 입력 (AI 단계용)
...
| 변수 | 용도 | 없으면 |
|------|------|--------|
| `OPENAI_API_KEY` | 자막 교정 · 장면 분석 · 금칙 판정 (`OPENAI_MODEL`, 기본 gpt-4o-mini) | 해당 단계 실패 처리, 나머지는 정상 진행 |
...
backend/    FastAPI — 파이프라인 · 검색 · 리포트
  app/      pipeline.py(8단계 병렬) · search.py(hybrid/keyword/vector/filter)
            ai.py(OpenAI) · media.py(ffmpeg) · stt.py(faster-whisper)
